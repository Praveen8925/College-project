...
<?php
             ...
             $tdate=date_format($tdate,"y-m-d");
               $SQL= "select * from workdiarys where SID='$userid' AND DATE >= '$fdate' AND DATE <= '$tdate'  ORDER BY DATE ASC,Hour ASC" ;
               $rs=mysql_query($SQL);  
                   $i=1;
    	        while($row=mysql_fetch_assoc($rs))
    	                  {
			    $date=$row["DATE"];
                 ...
                 print "<tr><td>$i</td><td>".$date."</td><td align='center'>".$row["DayOrder"]."</td>";
                 $remark=$row["Remark"];
                 if($remark=="OD")
                  print "<td colspan='5' align='center'>OD-".$row['session']."</td>";
                 else if($remark=="CL")
                  print "<td colspan='5' align='center'>Leave-".$row['session']."</td>";
                 else
                     {  
                 print "<td>".$row["Class"]."</td><td align='center'>".$row["Hour"]."</td>";
                 $sub=$row["subject"];
                 if($sub<>"")
                  {
