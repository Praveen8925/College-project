...
<?php
             ...
             $tdate=date_format($tdate,"y-m-d");
               $SQL= "select * from workdiarys where SID='$sid' AND DATE >= '$fdate' AND DATE <= '$tdate' and Asid <> ''  ORDER BY DATE ASC,Hour ASC" ;
               ...
               $rs=mysql_query($SQL);  
			    ...
			    $date=$row["DATE"];
                  ...
                  print "<tr><td>$i</td><td>".$date."</td><td align='center'>".$row["DayOrder"]."</td><td>".$row["Class"]."</td><td align='center'>".$row["Hour"]."</td>";
                       $remark=$row["Remark"];
                        $asid= $row["Asid"];
