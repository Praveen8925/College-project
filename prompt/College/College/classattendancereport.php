...
<center>
<?php
        $SQL= "select * from $tbs where Batch='$batch' and Course='$dept' and semester='$sem'";
		$rs1=mysql_query($SQL);  
	...
	print "<h1>No Data Found</h1>";
		...
		{
$t="select * from $tbs where Batch='$batch' and Course='$dept' and semester='$sem' and Date >= '$fdate' AND Date <= '$tdate'";
//print $t;
$r=mysql_query($t);  	
...
<?php         
    	...
    	while($row=mysql_fetch_assoc($rs))
    	{
            ...
            $reg=$row["RegNo"];
	        print "<tr><td align='center'>$i</td><td>".$row["RegNo"]."</td>";
			$i++;
?>
...
<?php 
		...
		$absent=0;
$fdate1=$fdate;
while(strtotime($fdate1)<=strtotime($tdate))
{	
		 $t1="select * from $tbs where Batch='$batch' and Course='$dept' and semester='$sem' and Date = '$fdate1' and (Ihour like '%$reg%' or IIhour like '%$reg%' or IIIhour like '%$reg%')";
$r1=mysql_query($t1); 
	$num1 = mysql_num_rows($r1);
	if($num1>0)
		$absent=$absent+0.5;
	$t2="select * from $tbs where Batch='$batch' and Course='$dept' and semester='$sem' and Date = '$fdate1' and (IVhour like '%$reg%' or Vhour like '%$reg%' or VIhour like '%$reg%')";
    $r2=mysql_query($t2);  	
